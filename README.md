...
GitHub Releases에서 `MoaIMF.dmg`를 내려받아 열고, `MoaIMF.app`을 `/Applications`로 복사합니다.

릴리스 앱은 아직 Developer ID 서명과 Apple 공증을 거치지 않은 빌드입니다. macOS가 실행을 차단하면, 앱을 신뢰할 수 있는 경우 다음 명령으로 격리 속성을 제거한 뒤 실행합니다.
...
`scripts/build-app.sh`가 만든 앱도 Developer ID 서명과 Apple 공증을 거치지 않은 빌드입니다. 다른 Mac에서 실행이 차단되면 위의 `xattr` 명령으로 격리 속성을 제거할 수 있습니다.
...
- 릴리스 앱은 아직 Developer ID 서명과 Apple 공증을 거치지 않았습니다.
- 자동 업데이트와 App Store 배포는 아직 없습니다.
