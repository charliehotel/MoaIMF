...
从 GitHub Releases 下载 `MoaIMF.dmg`，打开后将 `MoaIMF.app` 复制到 `/Applications`。发布版尚未经过 Developer ID 签名和 Apple 公证，因此 macOS 可能会阻止运行。如果你信任下载的应用，可以用以下命令移除隔离属性后启动。
...
`scripts/build-app.sh` 生成的应用也尚未经过 Developer ID 签名和 Apple 公证。如果 macOS 阻止运行，可以使用上面的 `xattr` 命令移除隔离属性。
